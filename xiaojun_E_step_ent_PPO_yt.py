    ...
    for step, batch in tqdm(enumerate(prompt_dataloader), total=len(prompt_dataloader)):
        ...
        with torch.no_grad():
            ...
            for i in range(seq.size(0)):
                # 先把最大下标拿到 CPU 上变成纯 Python int
                cur_len_val = seq[i].not_equal(tokenizer.pad_token_id).nonzero().max().item() - prompt_length
                # 计算长度奖励（纯浮点）
                length_bonus_float = 0.05 * float(min(cur_len_val, 40))
                # 转成与 reward_score 同设备（reward_score.device）、同 dtype 的张量
                length_bonus_t = torch.tensor(
                    length_bonus_float, 
                    device=reward_score.device, 
                    dtype=reward_score.dtype
                )
                # 再累加
                reward_score[i] = reward_score[i] + length_bonus_t
